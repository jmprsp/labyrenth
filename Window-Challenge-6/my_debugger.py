...
class debugger():
    ...
    def exception_handler_breakpoint(self):
        ...
        if not self.breakpoints.has_key(self.exception_address):
            ...
            if self.exception_address == 0x00401425:
				   #inject password
                   print "[*] Exception address: 0x%08x" % self.exception_address
                   self.write_process_memory(self.context.Ebx, self.password);
                   password = self.read_process_memory(self.context.Ebx,0xB)
                   print "[*] Injecting %s" % password
                   self.context = self.get_thread_context(h_thread=self.h_thread)
                   self.context.Eip -= 1
            elif self.exception_address == 0x0040143B:
			       #key found
                   print "[*] Exception address: 0x%08x" % self.exception_address
                   password = self.read_process_memory(self.context.Ebx,0xB)
                   print "[*] Injected Password: %s" % password
                   decoded = self.read_process_memory(self.context.Eax,0x40)
                   print "[*] Decoded %s" % decoded
                   self.context = self.get_thread_context(h_thread=self.h_thread)
                   self.context.Eip -= 1	  
                   exit()		
            elif self.exception_address == 0x00401458:
			       #exit if key is wrong
                   password = self.read_process_memory(self.context.Ebx,0xB)
                   print "[*] exiting [%s]" % password
                   exit()						     
            else:
                   self.context = self.get_thread_context(h_thread=self.h_thread)
                   self.context.Eip -= 1
